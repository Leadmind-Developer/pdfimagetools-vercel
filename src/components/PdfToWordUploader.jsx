...
const PdfToWordUploader = () => {
  ...
  const [uploads, setUploads] = useState([]); // Track progress and download links per file
  ...
  return (
    <div className="pdf-uploader">
      <h2>Convert PDF to Word (Batch Upload)</h2>
      ...
      {uploads.map((upload, idx) => (
        <div key={idx} className="upload-status">
          ...
          {upload.status === 'done' && upload.downloadUrl && (
            <a href={upload.downloadUrl} download={upload.name.replace('.pdf', '.docx')}>

...
const PptToPdfUploader = () => {
  ...
  const handleFilesSelect = (selectedFiles) => {
    ...
    if (validFiles.length === 0) {
      setError("Please select valid PowerPoint (.ppt or .pptx) files.");
      return;
    }
    setFiles(validFiles);
    setError("");
  ...
  return (
    <div className="pdf-uploader">
      <h2>Convert PowerPoint to PDF (Batch Upload)</h2>
        ...
        <div key={idx} className="upload-status">
          ...
          {upload.status === "done" && upload.downloadUrl && (
            <a
              ...
              download={upload.name.replace(/\.(ppt|pptx)$/i, ".pdf")}
            >
...
export default PptToPdfUploader;

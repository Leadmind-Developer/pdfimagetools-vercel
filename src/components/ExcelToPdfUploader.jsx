...
const ExcelToPdfUploader = () => {
  ...
  const handleFilesSelect = (selectedFiles) => {
    ...
    if (validFiles.length === 0) {
      setError("Please select valid Excel (.xls or .xlsx) files.");
      return;
    }
    setFiles(validFiles);
    setError("");
  ...
  return (
    <div className="pdf-uploader">
      <h2>Convert Excel to PDF (Batch Upload)</h2>
        ...
        <div key={idx} className="upload-status">
          ...
          {upload.status === "done" && upload.downloadUrl && (
            <a
              ...
              download={upload.name.replace(/\.(xls|xlsx)$/i, ".pdf")}
            >
...
export default ExcelToPdfUploader;
